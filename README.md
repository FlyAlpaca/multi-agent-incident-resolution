...
适用于服务故障日志、测试失败、回归、缺陷修复、多问题分诊，以及明确要求 diagnose/repair/review 的任务。不用于无缺陷证据的功能开发、普通重构或单纯代码解释。

## 工作流

1. 确认运行方式与 `debug`、`diagnose`、`repair` 或 `review` 范围。
2. 调查事实、触发条件、影响面和候选问题。
3. 诊断根因、违反的不变量、修复类型和验收标准。
...
每个子 Agent 使用任务专属的追加式 activity 文件；它只是多信号健康判断的一部分。无进展窗口单次最多等待 30 分钟（1800 秒），达到上限后先执行完整健康检查和非中断 checkpoint，不自动终止仍有可信完成路径的 Agent。任务正常结束后回显其终态，再清理该精确活动文件。整个流程正常完成时，仅清理当前已校验的 `RUN_ARTIFACT_DIR`；暂停、失败、阻塞、停止或取消时保留工件。

在自动全流程和单步确认模式中，所有实际子 Agent 均使用固定披露标签：

`<角色或稳定标识>（模型：<精确模型>；推理强度：<精确强度>）`

派发前同时说明有界任务；进度、替换、终态和最终总结复用同一标签。
