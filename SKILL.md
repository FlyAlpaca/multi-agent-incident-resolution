...
- Give every subagent the incident input, repository rules, workspace snapshot, run directory, bounded task, explicit authority boundary, assigned state/result paths, terminal handoff states, and the obligation to end its turn immediately after handoff.
