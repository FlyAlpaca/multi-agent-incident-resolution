...
- **协调者**：维护用户确认、事件范围、工件、问题台账、任务拆分（`tasks.yaml`）、路由记录、阶段状态和最终交付；在规划完成与验证失败两个节点主动重建 `active-context.md`，让旧推导退出活动上下文；不以协调判断替代独立证据。
...
- 每个实施 Agent 只接收：已校验的 `active-context.md`、本任务所属问题的简要输入、自己的 `tasks.yaml` 条目（任务 ID、文件范围、依赖、验收条件）、与本任务相关的 `plan.md` 片段、仓库规则、工作区快照、修复轮次与状态路径；
