...
- **集成员**：`POOLED` 实施的所有 Agent 退出后，唯一负责系统整合的写入者：在 `tasks.yaml` 的 `integration_scope` 内整理共享工作树、解决跨任务不一致、补齐预先规划的连接，并证明整体可装配；即使某个任务最终 `NO_CHANGE` 也不跳过已要求的集成。不重新设计、不改验收标准、不实现未选择问题，缺少写入范围或拆分有误时回退规划、方向问题回退诊断。
...
默认路由以 [SKILL.md](../SKILL.md#coordinate-agents) 为准，规划模式、实施池与集成门禁以 [workflow.md](../references/workflow.md#planning-mode) 为准；本文不复制这些决策规则。
...
交接只携带该角色完成有界任务所需的事件输入、仓库规则、权限、相关证据、产物与停止条件。实施派发只使用已批准的 `tasks.yaml` 条目；完整派发台账字段见 [artifacts.md](../references/artifacts.md)，状态与观察字段见 [subagent-state.md](../references/subagent-state.md)。不得夹带凭据或未脱敏的敏感输出。
