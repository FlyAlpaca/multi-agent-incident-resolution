...
- observation plan: initial wait, health-check cadence, and credible next milestone; apply the fixed thresholds from [subagent-state.md](subagent-state.md) rather than copying them into each row;
...
Reserve one `<RUN_ARTIFACT_DIR>/artifacts/tasks/<task-id>/state.md` path per active dispatch. Maintain `result.md` when the task completes and use `events.jsonl` only for a complex or abnormal task. [The subagent state protocol](subagent-state.md) defines the state schema, atomic-write rules, status vocabulary, and timeout decisions; the ledger records the path and observation plan without creating an auxiliary status or heartbeat file.
