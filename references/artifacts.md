...
- execution lifecycle: coordinator-owned `WORKER_LIFECYCLE: ACTIVE | TERMINAL_CONFIRMED | TERMINATION_FAILED` and `TERMINAL_CONFIRMATION: SELF_REPORTED | RUNTIME_STATUS | EXPLICIT_CLOSE | EXPLICIT_INTERRUPT | UNAVAILABLE`, governed by [subagent-state.md](subagent-state.md#terminal-handling). `EXPLICIT_INTERRUPT` is a lifecycle action, not a message: it is reserved for an explicit user cancellation, an independently enforced runtime/safety/authority stop, or force termination allowed by the fixed no-progress gate.
